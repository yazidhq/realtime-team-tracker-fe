...
export async function remove(id) {
  ...
  const body = await request(`/api/group_participant/${id}`, { method: "DELETE" });
  return body?.data || body;
}

export async function getAll() {
  const body = await request(`/api/group_participant`, { method: "GET" });
  return body?.data || body;
}

export async function getById(id) {
  const body = await request(`/api/group_participant/${id}`, { method: "GET" });
  return body?.data || body;
}
